...
function ProjectCard({
  ...
  const scale = useTransform(scrollYProgress, [0, 1], [Math.min(1, targetScale + 0.06), targetScale]);

  const cardStyle: MotionStyle = {
    scale,
    ["--stack" as string]: `${index * 28}px`,
  };

  return (
    <div ref={containerRef} className="relative h-[85vh]">
      <motion.div
        className="project-sticky-card sticky z-10 mx-auto max-w-6xl px-4 sm:px-6 md:px-8"
        style={cardStyle}
      >
