...
- **Motion**: **`motion`** — import from **`motion/react`** in **`"use client"`** components for interactive animation; respect reduced motion; static HTML uses CSS only (see `.cursor/rules/26-motion.mdc`).
