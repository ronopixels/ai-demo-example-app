...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: `${privacyPageMeta.title} · ${siteConfig.name}`,
    description: privacyPageMeta.description,
  },
};
  ...
  return (
    <div className="flex flex-1 flex-col bg-zinc-50 dark:bg-black">
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-12 sm:px-6 lg:py-16">
        <Breadcrumb
          className="mb-8"
          items={[{ label: "Home", href: routes.home }, { label: privacyPageMeta.title }]}
        />
        <PageHero
          title="Privacy policy"
          description="Placeholder policy for the Nexsas demo. Replace with a policy that matches your data practices."
        />
        <p className="mt-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100">
          Not legal advice. Consult qualified counsel before publishing.
        </p>
        ...
        <p className="mt-12 text-sm text-zinc-500 dark:text-zinc-400">
          <Link href={routes.marketing.terms} className="font-medium text-zinc-900 underline-offset-4 hover:underline dark:text-zinc-100">
            Terms of service
          </Link>
          {" · "}
